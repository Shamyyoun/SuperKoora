...
public class VideosAdapter extends RecyclerView.Adapter<VideosAdapter.ViewHolder> {
    ...
    public VideosAdapter(Context context, List<Video> data, int layoutResourceId) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (video.getId() != -1) {
            ...
            if (!video.getImage().isEmpty())
                Picasso.with(context).load(video.getImage()).into(holder.imageImage);
            // show or hide favorite icon
            holder.imageFavorite.setVisibility(video.isFavorite() ? View.VISIBLE : View.GONE);
